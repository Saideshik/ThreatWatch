...
import {
  ...
  ChevronRight, CheckCircle, AlertOctagon, Eye,
    ...
    >
      {isNew && (
        <span style={{ position: 'absolute', top: 8, right: 8, background: 'var(--critical)',
          ...
          fontFamily: 'var(--font-mono)', letterSpacing: '.05em' }}>NEW</span>
      )}
...
function AIResponse({ text }) {
  ...
  for (const sp of sectionPatterns) {
    const idx = remaining.toUpperCase().indexOf(sp.label)
    if (idx !== -1) hasSections = true
  }
  ...
  if (hasSections) {
    for (let i = 0; i < sectionPatterns.length; i++) {
      ...
      const startIdx = remaining.toUpperCase().indexOf(sp.label)
      if (startIdx === -1) continue
      const endIdx = next ? remaining.toUpperCase().indexOf(next.label) : remaining.length
      const content = remaining.slice(startIdx + sp.label.length, endIdx !== -1 ? endIdx : undefined).replace(/^[\s:]+/, '')
  ...
  if (sections.length === 0) {
    return (
      <div style={{ fontSize: 12, lineHeight: 1.8, color: '#86efac', whiteSpace: 'pre-line',
    ...
    <div className="anim-slide-in" style={{
      ...
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)',
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none',
          color: 'var(--text-secondary)', cursor: 'pointer', padding: 4, borderRadius: 4,
      ...
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
        ...
        <div>
          ...
          <p style={{ marginTop: 5, fontSize: 13, lineHeight: 1.55, color: 'var(--text-primary)', fontWeight: 500 }}>
            {alert.description}
          </p>
        </div>
        ...
        <div>
          ...
          <div style={{ marginTop: 7 }}>
            <AIResponse text={alert.ai_response} />
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    transition: 'all .15s ease', letterSpacing: '.04em',
...
export default function App() {
  ...
  const [isLive,        setIsLive]       = useState(true)   // ON by default
  ...
  useEffect(() => {
    ...
    if (!isLive) { setLiveConnected(false); return }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => { setIsLive(v => !v); setNewCount(0) }} style={{
            ...
            {isLive
              ? <><span className="blink"><Wifi size={11}/></span> LIVE</>
      ...
      <div style={{ padding: '8px 18px', borderBottom: '1px solid var(--border)',
        ...
        <Select value={filterAtk} onChange={e => setFilterAtk(e.target.value)}>
